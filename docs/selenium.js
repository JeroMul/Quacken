...
function setGeometry() {
  ...
  svg.setAttribute("viewBox", geometry.view_box);
...
function setConfig(hrm, vim) {
  const flavor = hrm ? "hrm" : "tt";
...
async function applyLayout(name) {
  ...
  setLayout(result.keymap);
}

const applyFlavor = () => {
  const hrm = document.getElementById("option-hrm").checked;
...
const applyConfig = () => {
  ...
  applyLayout(data.layout.toLowerCase());
  applyFlavor();
};
